...
- **`@ref` cross-references are resolved (v58 / ISS-64)** — inside the detail text, `@<id>` becomes a readable, light-blue reference to that action (e.g. `→ #142 Update wiring schedule`); `@img:<name>` becomes a light-blue reference to a picture of that name attached to the same row. Unresolvable references are kept as muted grey text (`→ #id [action not found]`, `→ #id <title> [deleted]`, or `→ <name> [image not found]`) so nothing is lost. This applies to **both** Word exports (Export actions and Export minutes). Resolution happens **only in the export** — the on-screen textarea and your saved data always keep the raw `@ref` token. A reference must be preceded by a space, line start, or `(` so an email like `a@123.com` is never mistaken for one.
- **Line breaks are preserved (v57 / ISS-91)** — multi-line detail text now exports with each line on its own line in Word, for **both** this Export minutes and the main Export actions Word export. Previously every Enter you typed was flattened to a space in the `.doc` file; the on-screen detail textarea was always correct and your saved data is untouched — only the exported document changed.
- **The action number is shown (v59 / ISS-92)** — each Word export (Export actions and Export minutes) now heads the action with its `#<id>` followed by the title (e.g. `#142 — Update wiring schedule`), matching the `@ref` hash style, so a printed or emailed document is easy to cross-reference back to the app. The Excel export already listed the id in its first column, so no change there.
