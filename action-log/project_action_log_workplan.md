...
| v43 | 2026-09-02 | Save/Working-folder workflow + standardized popups (ISS-80) | Standardized the Save workflow and all user-message popups. **Save Actions / Save Settings** moved out of the top-bar ⋯ menu into the right-panel Quick Actions (top-bar Save buttons removed, decision 5). New **Working Folder** model (decision 3/8/9): once a folder is chosen (any drive, not locked to Z:), Save writes `action.json`/`setup.json` **directly into it** via a new `writeIntoFolder()` — no re-prompt (decision 1); if no folder is set, Save opens the folder picker then writes immediately (decision 8); on Firefox / `file://` Save **downloads** the file + status hint. New standardized in-app message popup `#appMsg` (`appAlert` single-OK / `appConfirm` OK+Cancel) replaces native `confirm()`/`alert()`; navigation guard `confirmSaveBeforeLeave` blocks panel switches when dirty and shows "You have unsaved changes. Save before switching to another panel." (no auto-save, decision 2). New **Working Folder popup** `#wfPop` (`openWfPop`) shows the current folder + Choose button + explains "set folder ≠ save". Status bar `#statusbar` now shows working-folder name, last save time (`state.lastSavedAt`), amber unsaved dot + "● unsaved" text, and a browser-capability badge (✓ direct disk write / ⚠ download-only). Files: `js/core.js` (appAlert/appConfirm/openWfPop/lastSavedAt/guard), `js/io.js` (writeIntoFolder/saveTimeNow/rewrite writeDataFile/saveSetupFile/updateStatusbar/bindings), `index.html` (#wfPop/#appMsg/rpFolder; removed tbSaveActions/tbSaveSettings; tbFolder→"Working folder…"), `css/styles.css` (status-bar chips + .wf-current + .app-msg-*), `js/bootstrap.js` (wfPop close wiring), `USER_MANUAL.md` (Saving & working folder section). Status: **Implemented (2026-09-02, v43)** — pending `node --check` + browser smoke test + commit. |
| v44 | 2026-09-09 | Remove editor-top inline Save button (ISS-81) | The inline action editor's top action bar (`#edTop` / `renderActionTop`) had a **Save** button (`#aeSave`) that was the only path committing inline DOM edits into the action object AND writing the file. Per user request it is removed (no "Save" button at the top of the main panel). To avoid data loss, a new `flushInlineEdits()` (render.js) commits pending inline edits into the action object **without** writing the file; it is called automatically (1) at the start of `writeDataFile()` (io.js) so Save Actions / Ctrl+S captures inline edits, and (2) at the start of `renderActionsMain()` so switching actions never discards unsaved inline edits. `bindActionEditor` records `_openActionId` so flushInlineEdits targets the correct action even after `state.selection.actions` has changed. `markDirty` no longer toggles the removed button. The single save entry point is now the right-panel **Save Actions** / **Save Settings** + Ctrl/Cmd+S (consistent with ISS-80 decision 5). Files: `js/render.js` (remove `#aeSave` button + its two bindings + `markDirty` ref; add `flushInlineEdits` + `_openActionId`; call in `renderActionsMain`), `js/io.js` (call `flushInlineEdits` in `writeDataFile`). Status: **Implemented (2026-09-09, v44)** — pending `node --check` (render.js/io.js) + browser smoke test (edit inline → Save Actions persists; switch action mid-edit persists; no top Save button) + commit. |
| v45 | 2026-09-09 | Working Folder popup: show default deploy path (ISS-82) | User asked to make the launcher's deploy target visible inside the Working Folder popup so the folder name in the status bar can be reconciled with the real on-disk location. Added a `.wf-deploy-hint` paragraph in `openWfPop` (core.js) showing `Z:\7. Engineering\01-Eng Mgmt\01 Action Log` as the **default deploy folder** (where the launcher serves the app from), clarifying that Save writes to the chosen working folder (which may be a different drive) and that this path is a read-only reference, not part of the save logic. CSS `.wf-deploy-hint` + `.wf-deploy-hint code` added in styles.css. Files: `js/core.js`, `css/styles.css`. Status: **Implemented (2026-09-09, v45)** — pending `node --check js/core.js` + browser smoke test (popup shows deploy path; choosing a folder still works) + commit. |
| v46 | 2026-09-09 | Working Folder popup: click-to-copy deploy path (ISS-83) | Extends ISS-82: the deploy-path hint in `openWfPop` (core.js) is now **click-to-copy**. The path text (`#wfDeployPath`) and a new **Copy** button (`#wfCopyPath`) both call a new `copyText()` helper; on success a `✓ Copied` label (`#wfCopyOk`) flashes for ~1.6s and a toast confirms. `copyText()` uses `navigator.clipboard.writeText` when available (secure contexts incl. `http://localhost`) and falls back to a hidden `<textarea>` + `document.execCommand('copy')` for non-secure contexts (Firefox/`file://`). Path is stored once in the module constant `DEPLOY_PATH`. CSS `.wf-deploy-hint code.wf-path` (clickable, hover accent), `.wf-copy` (button), `.wf-copy-ok` (success label) added in styles.css. Files: `js/core.js`, `css/styles.css`. Status: **Implemented (2026-09-09, v46)** — pending `node --check js/core.js` + browser smoke test (click path or Copy → clipboard has full path; ✓ Copied flashes; Choose folder still works) + commit. |
| v47 | 2026-09-09 | File-link label: drop `file:///` + `%20` from display (ISS-84) | Bug: when a user pastes a `file:///` folder URL (e.g. `file:///Z:/7.%20Engineering/01-Eng%20Mgmt/03%20Master%20List/01-PPP`) into the attachment popover's "Add link" box, the rendered link still exposed the raw `file:///` prefix and `%20` escapes — in the chip **tooltip** (`title="${normalizeLinkSrc(im.src)}"`) and as the stored `name` for new links. Added `prettyLinkName(src)` (render.js) that strips the `file://` scheme, `decodeURIComponent`s `%20`→space, and restores backslashes, returning a clean Windows path (`Z:\7. Engineering\01-Eng Mgmt\03 Master List\01-PPP`); web/blob/data sources keep the legacy last-segment behaviour. The "Add link" handler now stores `name: prettyLinkName(v)` and `renderChips` shows `title="${prettyLinkName(im.src)}"`, so the visible text + tooltip never show `file:///` or `%20`. The link **href** still uses `normalizeLinkSrc(im.src)` (a valid `file://` URL) so the folder still opens on click. Also hardened `normalizeLinkSrc` to `%20`-encode literal spaces in the Windows-path branch so space-containing paths open reliably. Files: `js/render.js`. Status: **Implemented (2026-09-09, v47)** — pending `node --check js/render.js` + browser smoke test (paste file:// folder URL → chip shows clean path + opens on click; no file:// or %20 in text/tooltip) + commit. |
| v48 | 2026-09-08 | Save-lifecycle feedback + status-bar unsaved fix (ISS-85) | User reported (a) no "saving in progress" indicator when Saving Actions/Settings over a VPN-mapped network drive, allowing repeated clicks during the slow write, and (b) the status bar did not show "● unsaved" on changes. Root cause (a): `writeDataFile`/`saveSetupFile` are async with no visual state and save buttons only disable on dirty (no in-flight feedback, re-entry possible). Root cause (b): `updateStatusbar()` was only called on save-complete/folder-set/auto-load while the dirty setters (`markDirty`/`markDataDirty`/`markSetupDirty`) only called `updateSaveButtons()`; `syncTopbarUnsaved` also omitted `edDirty`. Fix: a `saving` flag guards re-entry (`if(saving) return`) and drives a blue pulsing dot + "Saving…" spinner (status bar) + a spinner on all four save buttons (`pointer-events:none` keeps them full-opacity, blocks re-click); `updateStatusbar()` now fires on every dirty change so "● unsaved" appears immediately; three-state CSS — Saving (accent + spinner) > `⚠ Save failed` (red, auto-clears 4s) > `● unsaved` (amber) — with `prefers-reduced-motion` support; `syncTopbarUnsaved` now includes `edDirty`; dead `#aeSave` code removed from `updateSaveButtons`. New `setSaving(on)` / `showSaveError(msg)`. Files: `js/core.js`, `js/io.js`, `js/render.js`, `css/styles.css`. Status: **Implemented (2026-09-08, v48)** — pending `node --check` (core.js/io.js/render.js) + browser smoke test (edit → status bar "● unsaved" at once; Save over slow VPN shows blue "Saving…" + button spinner, no duplicate save; success → "Saved <time>"; failure → red "⚠ Save failed"; top-bar "● unsaved" reflects inline edits) + commit. |
| v49 | 2026-09-09 | Status bar mislabels fresh-open as "Not saved yet" (ISS-86) | Bug: on first open the save-time chip read "Not saved yet" even though a file was just loaded and the in-memory data equals the on-disk baseline — so it wrongly implied pending unsaved edits. Root cause: `savedTxt` in `updateStatusbar()` (io.js) fell back to "Not saved yet" whenever `state.lastSavedAt` was empty, but `lastSavedAt` is only set after an actual Save, so every fresh open (source external/imported/embedded) showed "Not saved yet". Fix: `savedTxt` now returns "Loaded" when a dataset was loaded (`src !== 'none'`) and `lastSavedAt` is empty; "Not saved yet" is reserved for the genuine empty state (`src === 'none'`, no file opened). The chip also gets a green `.sb-loaded` style (same palette as `sb-ext`/`sb-cap-ok`) with a tooltip "Loaded from file — matches disk, no unsaved changes", so it never reads as an error. Files: `js/io.js`, `css/styles.css`. Status: **Implemented (2026-09-09, v49)** — pending `node --check js/io.js` + browser smoke test (fresh open of action.json → chip shows green "Loaded", no "● unsaved"; truly empty/no-file start → still "Not saved yet"; after a Save → "Saved <time>") + commit. |
| v50 | 2026-09-09 | Status bar reorganised into per-message panels (ISS-87) | User review (do-not-update pass): the status bar was a flat run of unlabeled fragments — "from file"/"Working Folder/File/Status" noise that read as a jumble. Follow-up to ISS-85/86: each status message is now its own bordered **panel** on the (dark) bar — `[file]` (coloured source dot + filename) · `[folder]` (SVG folder icon replacing the emoji + working-folder name) · `[save-state]` (one unambiguous phrase: `Saving…` / `Save failed` / `● N unsaved` / `Loaded` / `Saved <time>`, with `from file` as a muted suffix) · `[counts]`. The browser-capability badge ("direct disk write"/"download-only") moves into the dot's tooltip so it stops crowding the right edge; the migration tag (`⚠ legacy → v1`) becomes its own orange panel. Panels use translucent chips (rgba white 0.06 fill, 0.14 border, 7px radius) suited to the dark bar; the folder panel flexes (ellipsis) and the counts panel is pushed right (`margin-left:auto`). A `max-width:900px` media query hides the folder + counts panels first so the save-state panel never clips. Files: `js/io.js`, `css/styles.css`. Status: **Implemented (2026-09-09, v50)** — pending `node --check js/io.js` + browser smoke test (bar shows 4 distinct panels; fresh open → green "Loaded · from file"; edit → amber "● N unsaved"; Save → blue "Saving…" + button spinner; success → "Saved <time>"; narrow ≤900px → folder+counts drop, save-state stays) + commit. |
| v51 | 2026-09-09 | First Save fails with no fallback / data loss (ISS-88) | User reported fresh-load Save showed `✓ Direct write` then `Save failed: <details>` with no file written. Root cause: capability panel reflected only browser API support (not an actual writable folder); `loadDataDir()` restores a previously-chosen (possibly VPN) folder into `dataDirHandle` before first Save; on `writeIntoFolder` throw all four write-failure branches only errored and gave up (silent data loss). Fix: every `writeIntoFolder` failure in `writeDataFile`/`saveSetupFile` now falls back to `downloadJson` with a cause toast, clearing error/dirty state — no silent loss. Files: `js/io.js`. Status: **Implemented (2026-09-09, v51)** — pending `node --check js/io.js` + browser smoke test + commit. |
| v52 | 2026-09-10 | Save workflow unified + capability 3-state + save-result wording (ISS-89) | User reported (a) status bar showed `✓ Direct write` on first load with no folder, (b) Edge first Save only downloaded, (c) picking a folder triggered a non-standard system permission window. Review found P1–P5. Fix (A–D): (A) capability panel 3-state — folder set → green `✓ Direct write`; API but no folder → amber `⚠ Set folder`; no API → amber `⚠ Download-only` (no false green); (B) Save with no folder opens the standardized `openWfPop` and awaits it (dismiss → data stays unsaved, toast, no silent download); `openWfPop` returns a Promise resolved via a MutationObserver on popup close (Save never hangs); `setDataFolder` returns a boolean; (C) `openWfPop` explains the native browser permission prompt (`.wf-perm-hint`) so it no longer reads as a stray window; (D) status bar distinguishes `Saved <time>` (disk) vs `Downloaded <time>` (amber, fallback) via `state.lastSavedVia`. All three folder entry points (rpFolder/tbFolder/dtFolder) now use `openWfPop`. Files: `js/core.js`, `js/io.js`, `js/settings.js`, `css/styles.css`. Status: **Implemented (2026-09-10, v52)** — pending `node --check` (core.js/io.js/settings.js) + browser smoke test (fresh Edge → amber `⚠ Set folder`; Save → standardized popup; dismiss → no download + toast; choose → direct write; native prompt explained in-popup; `Saved` vs `Downloaded`) + commit. |
| v53 | 2026-09-10 | Save buttons stay clickable when only `edDirty` is true (ISS-90 #1) | During a slow in-flight save (VPN/network folder, or while the working-folder popup is open), `flushInlineEdits()` commits the DOM into the action object **before** the `await`, but `writeDataFile`/`saveSetupFile` clear `state.dataDirty` in their `finally`-block — while a new inline edit typed during the `await` can leave `edDirty=true, dataDirty=false`. `updateSaveButtons()` gated Save Actions on `state.dataDirty` alone, so the button ended up **disabled** while the status bar showed `● unsaved` — an inconsistent state with a silent-loss path (switching panel flushes the DOM edit into `a` without re-dirtying, so the edit was never saved and closing the page raised no `beforeunload` warning). Fix (ISS-90 #1 only): Save Actions (`tbSaveActions`/`rpSaveActions`) now enable on `state.dataDirty || edDirty`, closing the inconsistent state and the silent-loss path. (ISS-90 #2 — re-serializing the data **after** the `await` so a slow save captures the latest edit rather than the pre-edit snapshot — is deferred, pending user approval.) Files: `js/core.js` (`updateSaveButtons`). Status: **Implemented (2026-09-10, v53)** — pending `node --check js/core.js` + browser smoke test (slow Save + edit mid-flight → Save Actions stays clickable, status consistent; switch panel then Save persists the edit) + commit. |
| v54 | 2026-09-10 | Slow save writes completion-time data, not pre-edit snapshot (ISS-90 #2) | User approved ISS-90 #2 (was deferred after v53). During a slow in-flight save (VPN/network-folder `writeIntoFolder`, or while the `openWfPop` working-folder popup is open), the old code committed the DOM into the action object via `flushInlineEdits()` **before** the `await` and serialized `state.actions` once at click time — so an inline edit typed during the `await` was committed into `a` but the file was written from the **pre-edit** snapshot, silently losing that edit until the next Save. Fix (ISS-90 #2): `writeDataFile` (io.js) no longer snapshots at click time — it builds the serialized text lazily via a `snapshot()` helper **right before each write/download, after any preceding await**, so the file captures the DOM as it was at completion (including mid-flight edits). `snapshot()` still runs `flushInlineEdits()` so pending inline edits are committed; `updateSaveButtons` already keeps Save Actions clickable via `state.dataDirty || edDirty` (ISS-90 #1) so the user can always re-Save. `saveSetupFile` is unchanged (settings have no inline edits). Files: `js/io.js` (`writeDataFile`). | §5.1 / §6 | **Implemented (2026-09-10, v54)** — pending `node --check js/io.js` + browser smoke test (slow Save + edit mid-flight → saved action.json contains the latest edit, not the pre-edit snapshot) + commit. |
| v55 | 2026-09-11 | ae-log filter/sort toolbar (ISS-62) | View-only filter (Type / By / Status / From / To) + sort (Date / Type / By / Status, asc/desc) toolbar above `#aeLog`. Filtering hides rows without removing them from the DOM (Save never persists a filter); sorting reorders the DOM for display only and `collectLogRows()` restores data order on flush so a sort is never saved; ↑/↓ reorder disabled while a sort is active. Files: `js/render.js`, `css/styles.css`. Status: **Implemented (2026-09-11, v55)** — pending `node --check js/render.js` + browser smoke test + commit. |
| v56 | 2026-09-11 | Export minutes (ISS-63) | **Export minutes** button (appended to `#edTop` in `bindActionEditor`) exports the open action's dated detail log as Word (title + metadata block + 6-column log table: Date / Type / Action by / Due / Status / Detail) via `msoDownload`. Reuses the `exportWord` metadata block + detail-log field resolvers; exports the **current ISS-62 filter/sort view** (`exportOrderedDetailLog` reads live DOM rows, skipping filtered-out rows, in view display order). Word only. Files: `js/render.js`, `js/report.js`. Status: **Implemented (2026-09-11, v56)** — pending `node --check js/render.js js/report.js` + browser smoke test (Word minutes; exports filtered/sorted rows; @ref deferred to ISS-64) + commit. |

| v57 | 2026-09-18 | Export detail-log line breaks in Word (ISS-91) | Bug: Word export flattened each action's detail-log line breaks into a single space — the generated `<td>` used only `esc(r.text)` (which escapes HTML special chars but performs no newline→`<br/>` conversion) and carried no `white-space:pre`, so Word's HTML-whitespace rule collapsed every Enter into a space. On-screen rows are `<textarea>` (native newline preservation), so only the export was wrong. Fix: in both `detailLogRows` (exportWord) and `minutesLogRows` (exportMinutes), emit `esc(r.text||'').replace(/\r\n|\r|\n/g,'<br/>')` — added AFTER `esc()` so the `<br/>` is not re-escaped, while `& < >` stay safe. Files: `js/report.js`. Status: **Implemented (2026-09-18, v57)** — pending `node --check js/report.js` + browser smoke test (multi-line detail → Word shows each line on its own line, in both Export actions and Export minutes; data unchanged) + commit. |
| v58 | 2026-09-18 | @ref resolution in Word export (ISS-64) | Lightweight `@ref` in detail-log text — `@<id>` → action, `@img:<name>` → picture on the same row — resolved into readable text **only at export** (Export actions Word via `detailLogRows`, Export minutes via `minutesLogRows`) through `expandRefs(text, row)`; raw token retained when unresolvable (muted grey `[action not found]` / `[deleted]` / `[image not found]`). New helpers `findActionById` / `resolveActionRef` / `resolveImgRef` / `expandRefs` in `js/report.js`; resolution is opt-in (`resolve` flag, default off) so any on-screen caller stays raw; a token boundary guard requires `@` to follow a space, line start, or `(` so emails like `a@123.com` are never mistaken for a ref. Resolved refs highlighted light blue (`#185FA5` on `#E6F1FB`); `@img:` uses precise (case-sensitive) name match. Files: `js/report.js`. Status: **Implemented (2026-09-18, v58)** — pending `node --check js/report.js` + browser smoke test (@142 resolves, @999 muted, @img:name resolves / missing muted, in both Word exports; data unchanged; Excel unaffected) + commit. |
| v59 | 2026-09-18 | Show action number on Word export heading (ISS-92) | The Word exports (Export actions via `exportWord`, Export minutes via `exportMinutes`) headed each action section by its title only, so the action's own id never appeared on the page (only the Excel export had an `ID` column). Fix: prefix the section `<h2>` with `#<id> — ` in both `exportWord` (L114) and `exportMinutes` (L161), matching the ISS-64 `@ref` hash style. Files: `js/report.js`. Status: **Implemented (2026-09-18, v59)** — pending `node --check js/report.js` + browser smoke test (both Word exports show `#id — title` heading; Excel unaffected; data unchanged) + commit. |
...
### [2026-09-10] Issue — Save workflow + status bar (ISS-89)
- **Reported by / trigger**: User: on first open (Edge) the status bar showed `✓ Direct write` even with no folder; first Save only downloaded; picking a working folder popped a non-standard system permission window. Review-only pass (do-not-update) mapped P1–P5.
- **Detail**: capability panel conflated browser API support with an actual writable folder; Save hijacked the raw system folder picker (bypassing the standardized `openWfPop`); the native File System Access permission prompt is browser-rendered so it looked non-standard; status bar couldn't distinguish a disk write from a download fallback.
- **Decision / status**: Implemented A–D — capability 3-state, Save → standardized popup (await), in-popup permission explanation, `Saved` vs `Downloaded` wording. Files: core.js/io.js/settings.js/styles.css. (Done — pending node --check + browser smoke test + commit)
- **Related**: §5.1 / §6

### [2026-09-10] Issue — Save buttons disabled while status shows unsaved (ISS-90 #1)
- **Reported by / trigger**: User asked to evaluate Save for all cases, esp. "while saving is in progress, should not allow user to edit, otherwise Save button is deactivated and user can never save again". Traced the three-state correlation (`saving` / `edDirty` / `dataDirty`) and found an inconsistent state: during a slow in-flight save a new inline edit can leave `edDirty=true, dataDirty=false` after the save's `finally` clears `dataDirty`; `updateSaveButtons` gated Save Actions on `dataDirty` alone → button disabled while status bar shows `● unsaved`.
- **Detail**: `flushInlineEdits()` commits the DOM into the action object before the `await` in `writeDataFile`/`saveSetupFile`; the save clears `state.dataDirty` in its `finally`-block. If the user types during the `await` (VPN/network-folder write, or while the working-folder popup is open), `markDirty` sets `edDirty=true` (dataDirty already true), but at completion `dataDirty=false` is set while `edDirty` is untouched → `saving=false, dataDirty=false, edDirty=true`. `updateSaveButtons` enabled Save Actions from `state.dataDirty` only, so the button was disabled with `● unsaved` showing; switching panel then flushes the DOM edit into `a` without re-dirtying → silent edit loss + no `beforeunload` warning on close. Note: `saving` and the dirty flags are orthogonal — `saving` never reads/writes them, and always resets via `try/finally`, so a genuine deadlock (edits locked + button disabled forever) does NOT occur; the bug is the inconsistent UI + silent-loss path, not a permanent lock.
- **Decision / status**: Implemented #1 (v53) — Save Actions (`tbSaveActions`/`rpSaveActions`) now enable on `state.dataDirty || edDirty`, closing the inconsistent state and the silent-loss path. Files: `js/core.js` (`updateSaveButtons`). **#2 implemented (2026-09-10, v54)** — see next entry. (Done — pending `node --check js/core.js` + browser smoke test + commit)
- **Related**: §5.1 / §6, ISS-89

### [2026-09-10] Issue — Slow save writes completion-time data, not pre-edit snapshot (ISS-90 #2)
- **Reported by / trigger**: User approved ISS-90 #2 ("proceed #1 first" → then "next phase B, 6 is not required"). Follow-up to ISS-90 #1: even with the button kept clickable, the saved file still held the pre-edit snapshot.
- **Detail**: `writeDataFile` (io.js) committed the DOM into the action object via `flushInlineEdits()` and serialized `state.actions` **once at click time**, before the `await writeIntoFolder` (or `await openWfPop`). So an inline edit typed during a slow in-flight save was committed into the action object (`a`) but the file was written from the pre-edit snapshot — silently lost until the next Save. ISS-90 #1 only fixed the button/stale-state half; this fixes the data half.
- **Decision / status**: Implemented (v54) — `writeDataFile` no longer snapshots at click time; it builds the serialized text lazily via a `snapshot()` helper **right before each write/download, after any preceding await**, so the file captures the DOM at completion (including mid-flight edits). `snapshot()` still runs `flushInlineEdits()` so pending inline edits are committed; `updateSaveButtons` (ISS-90 #1) keeps Save Actions clickable so the user can always re-Save. `saveSetupFile` unchanged (settings have no inline edits). Files: `js/io.js` (`writeDataFile`). (Done — pending `node --check js/io.js` + browser smoke test + commit)
- **Related**: §5.1 / §6, ISS-89, ISS-90 #1

### [2026-09-10] Phase A closed + ISS-74 header-scope question decided not required
- **Reported by / trigger**: User: "proceed phase a now" then "ok now, update workplan, then next phase B (6 is not required)".
- **Detail**: Phase A = verify + commit the v38→v53 backlog (ISS-67…ISS-90 #1). Agent sandbox shells (Bash + PowerShell) were down ("Connection lost"), so `node --check` / browser / `git` could not run in-sandbox; the three files edited this chain (core.js / io.js / settings.js) were manually read-back and confirmed intact. Remaining modules + `apply_v38.js` and the actual commit are user-side (local). Step 6 of the earlier plan — "confirm whether the grey 'Description — dated detail log' label should also pin with the column header (ISS-74 header-scope question)" — is explicitly **not required** per user; that label stays non-sticky (only the Date/Detail/Meta column header pins, per v40/ISS-74).
- **Decision / status**: Phase A accepted (user to run local `node --check` + smoke test + commit). ISS-74 header-scope question: **decided not required** — no change. Next: Phase B = ISS-90 #2 (done this turn, v54).
- **Related**: ISS-74, ISS-89, ISS-90

### [2026-09-09] Issue — Status bar reads "Not saved yet" on first open (ISS-86)
- **Reported by / trigger**: User opened action log fresh; status bar showed "Not saved yet" although a file had just been loaded — misleading, since the in-memory data equals the on-disk baseline (no pending edits).
- **Detail**: `updateStatusbar()` (io.js) computed `savedTxt = state.lastSavedAt ? ('Saved …') : 'Not saved yet'`. `lastSavedAt` is only set after an actual Save, so every fresh open (source external/imported/embedded) showed "Not saved yet", implying unsaved work that did not exist.
- **Decision / status**: On fresh open, the loaded file IS the saved baseline → show **"Loaded"** (green) when `src !== 'none'` and `lastSavedAt` empty; reserve "Not saved yet" for the genuine empty state (`src === 'none'`). Added `.sb-loaded` green style + clarifying tooltip. Implemented (v49). Pending `node --check js/io.js` + browser smoke test + commit.
- **Related**: §5.1 / §6

### [2026-09-09] Issue — Status bar reorganised into per-message panels (ISS-87)
- **Reported by / trigger**: User review (do-not-update pass) of the status bar: the bar was a flat run of unlabeled fragments ("from file"/"Working Folder/File/Status" noise) that read as a jumble; user suggested "show panel for each message, or use divider".
- **Detail**: Each status message is now its own bordered panel on the (dark) bar — `[file]` (source dot + filename), `[folder]` (SVG folder icon + working-folder name), `[save-state]` (one phrase: Saving… / Save failed / ● N unsaved / Loaded / Saved <time>, with `from file` as a muted suffix), `[counts]`. Capability badge folded into the dot tooltip; migration tag is its own orange panel. Translucent chip styling suits the dark bar; folder panel flexes (ellipsis), counts pushed right; `max-width:900px` hides folder+counts first so save-state never clips.
- **Decision / status**: Implemented (v50) via `updateStatusbar()` rewrite (io.js) + `.sb-panel*` CSS (styles.css). Pending `node --check js/io.js` + browser smoke test + commit.
- **Related**: §5.1 / §6
...
### [2026-09-08] Implementation — Attachment popover "Card C" hybrid layout (ISS-78)
- Reported by / trigger: user design review (2026-09-08, Chinese) of the `#aeAttachPop` popover found 4 issues — two tip-text bugs (`\f` escape in `C:\folder`; false `file://` "opens" promise), inconsistent alignment between the Pictures and File-links groups, and the core `file://` limitation. User then evaluated an "all buttons at the bottom" variant, and approved the agent's **Card C** recommendation (content grouped inline; only Done in the footer; references GitLab / Primer / macOS modal patterns). Then: "use your recommendation C."
- Detail: rebuilt `openAttachPop` body as two cards (`.ae-attach-card`) — **Pictures** (`+ Add pictures` in the header) and **File links** (`Browse files` in the header; a paste-URL + `Add link` row; an honest tip). CSS `.ae-attach*` block rewritten as card styling; `renderChips` unchanged. Also escaped the `C:\folder` example and rewrote the tip to be honest about `file://` vs server-relative paths. Files: `js/render.js`, `css/styles.css`.
- Decision / status: **Implemented (2026-09-08)** — pending `node --check js/render.js` + browser smoke test (popover shows two cards; header actions right-aligned; Done only in footer; empty cards visible; tip text correct + honest `file://` guidance) + commit.
- Related: §16.4 ISS-78; supersedes the loose inline layout from ISS-77.

### [2026-09-08] Requirement/Implementation — show action number (`#id`) in the left tree node (ISS-79)
- Reported by / trigger: user asked to show the action's number in the tree node ("show action number in tree node in left side panel"). Design was proposed and confirmed earlier (Action ID `#NN`, placed **left of the title**); user then reported "i did not see the action number" because the change had only been proposed, not applied (awaiting the go-ahead). Implemented on the follow-up.
- Detail:
  - `js/render.js` `renderActionNode(a, depth, …)`: emits `<span class="tnum">#${a.id}</span>` immediately after the chevron/dot (`${lead}`) and before `<span class="tname">` — so the number sits on the **left, before the title** for every action row (top-level and nested sub-actions).
  - `css/styles.css`: added `.tnum` — muted text on `--chip` at rest; when the row is selected (`.tnode.action.sel`) the chip becomes solid `--accent` background + white text, matching the existing `.sel` light-tint treatment. Monospace, `flex:none`, `margin-left:6px` so it clears the chevron/dot.
  - Parent rows keep their existing `∑ N` rollup badge (separate chip, after the title). Works in filtered views. No data-model or `io.js` migration — purely presentational; the `#id` already drives the subhead, dependency editor, and exports, so the tree now matches those references.
- Decision / status: **Implemented (2026-09-08)** — pending `node --check js/render.js` + browser smoke test (#id shows left of each title; parents still show `∑ N`; selected row highlights chip in accent; tracks new/reordered actions) + commit.
- Related: §16.4 ISS-79.

### [2026-09-02] Requirement/Implementation — Save/Working-folder workflow + standardized popups (ISS-80)
- Reported by / trigger: user reviewed the Save workflow and asked to standardize all user-message popups and make the Working Folder explicit. Ten decisions locked across the review (Chinese evals + mockups + workflow chart), then "proceed now".
- Decisions (locked): (1) folder known → Save writes directly, no re-prompt; folder unknown → ask to choose; (2) stay on same page, no auto-save; (3) any drive, not locked to Z:; (4) status bar shows working-folder set/unset; (5) top-bar Save buttons removed — Save lives in right-panel Quick Actions + Ctrl/Cmd+S; (6) status bar shows browser-limit messages; (7) folder button clearly means "set folder, not save"; (8) no folder → open picker then save immediately; (9) standardized `#appMsg` popup (`appAlert` OK-only / `appConfirm` OK+Cancel) replaces native `confirm()`/`alert()`; (10) navigation guard blocks panel switch when dirty and informs "Save before switching to another panel" (OK alone sufficient; Cancel meaningless).
- Changes:
  - `js/core.js`: added `state.lastSavedAt`; rewrote `confirmSaveBeforeLeave` (covers `edDirty`, no auto-save, uses `appAlert`); added `appAlert` / `appConfirm` / `openWfPop`.
  - `js/io.js`: new `writeIntoFolder(folder,name,text)` (direct in-folder write, no picker) + `saveTimeNow()`; rewrote `writeDataFile` / `saveSetupFile` (folder-aware: direct write when `dataDirHandle` set; choose-then-save when not; download fallback on Firefox/file:// with status hint); rewrote `updateStatusbar` (working-folder name + last save time + amber unsaved dot + "● unsaved" + browser-capability badge ✓ direct disk write / ⚠ download-only); removed `tbSaveActions`/`tbSaveSettings` bindings; `tbFolder` → `openWfPop`; added `rpFolder` → `openWfPop`.
  - `index.html`: added `#wfPop` (Working Folder modal) + `#appMsg` (message modal); added `rpFolder` "Select Working Folder" button; removed `tbSaveActions`/`tbSaveSettings`; `tbFolder` label → "Working folder…".
  - `css/styles.css`: status-bar chips `.sb-saved` / `.sb-cap` / `.sb-cap-ok` / `.sb-cap-no` / `.sb-unsaved-txt` + amber `.sb-dot.sb-unsaved`; `.wf-current` / `.wf-k` / `.wf-v`; `.app-msg-modal` / `.app-msg-text`.
  - `js/bootstrap.js`: added `wfPop` to backdrop-click-close list; wired `wfPopClose` / `wfPopDone`.
  - `USER_MANUAL.md`: new "Saving & working folder" section (save buttons, working folder, browser-limit table, status-bar readout, navigation guard).
- Decision / status: **Implemented (2026-09-02, v43)** — pending `node --check` on all touched JS (core.js / io.js / bootstrap.js) + browser smoke test (save direct into chosen folder; folder picker then save; download fallback on Firefox/file://; status bar shows folder/time/unsaved/capability; standardized popups; navigation guard blocks switch) + commit.
- Related: §2 v43; §16.4 ISS-80; §5.1, §5.5 (Approach A/B save behaviour).

*(New issues/requirements/updates get appended here following the convention above.)*

### [2026-09-08] Requirement/Implementation — save-lifecycle feedback + status-bar "unsaved" fix (ISS-85)
- Reported by / trigger: user reported (Chinese, 2026-09-08) that (a) clicking Save Actions / Save Settings over a VPN-mapped network drive shows no "saving in progress" indicator and allows repeated clicks during the slow write, and (b) the status bar did not show "● unsaved" when changes were made. Asked for a thorough UI-design-professional review of all save/status-bar details.
- Root cause: (a) `writeDataFile`/`saveSetupFile` are async with no visual state; the save buttons are only disabled by the dirty flag, so a slow VPN write has no feedback and re-entry is possible. (b) `updateStatusbar()` was only called on save-complete / folder-set / auto-load, while the dirty setters (`markDirty`/`markDataDirty`/`markSetupDirty`) only called `updateSaveButtons()` — never `updateStatusbar()`. `syncTopbarUnsaved` also omitted `edDirty`, so inline-editor edits did not light the top-bar "● unsaved".
- Detail / fix:
  - `js/core.js`: added `saving` + `saveErrorMsg` state flags next to `edDirty`; `markDataDirty`/`markSetupDirty` now also call `updateStatusbar()`; `updateSaveButtons()` rewritten — while `saving` is true it adds `.is-saving` to all four save buttons (`tbSaveActions`/`rpSaveActions`/`tbSaveSettings`/`rpSaveSettings`) and returns early (blocks re-enable); removed the dead `#aeSave` branch (the inline Save button was already gone in ISS-81); added `setSaving(on)` + `showSaveError(msg)` helpers.
  - `js/io.js`: `writeDataFile`/`saveSetupFile` wrapped in `setSaving(true)` … `try { … } finally { setSaving(false) }` with an early `if(saving) return;` re-entry guard; failures call `showSaveError(...)` (was a plain `toast`); every success path clears `state.dataDirty`/`state.setupDirty`, stamps `state.lastSavedAt`, and clears `saveErrorMsg`. `updateStatusbar()` dot now encodes the saving state (blue `sb-saving` pulse) and renders a single priority chip — Saving > Save failed > unsaved > none; `syncTopbarUnsaved()` dirty check now includes `edDirty`.
  - `js/render.js`: `markDirty` now calls `updateStatusbar()` so an inline-editor edit immediately shows "● unsaved".
  - `css/styles.css`: added the ISS-85 save-lifecycle block — blue pulsing saving dot, `sb-spin` spinner, red `.sb-err-txt`, and `.is-saving` button spinners (kept full-opacity via `pointer-events:none`); `prefers-reduced-motion: reduce` disables the spinners/pulse.
- Decision / status: **Implemented (2026-09-08, v48)** — pending `node --check` (core.js/io.js/render.js) + browser smoke test (edit → status bar shows "● unsaved" immediately; Save Actions over slow VPN shows blue "Saving…" + button spinner and blocks a second save; success → "Saved <time>"; failure → red "⚠ Save failed" then auto-clears; top-bar "● unsaved" reflects inline edits) + commit.
- Related: §2 v48; §16.4 ISS-85.
...
### [2026-09-10] Housekeeping — Phase C: archive apply/scratch files + user-manual update
- Archived four intermediate/interim files from `action-log/` root into `arch/`: `v37_apply.md`, `v38_apply.md` (manual find→replace apply guides, both long applied), `apply_v38.js` (the idempotent patch script for the v38 defect batch, long applied), and `OVERVIEW.md` (a stray session note with a wrong username path — intermediate, not needed). Per project convention "archive, never delete": content preserved; originals removed from root via `move` (run locally — agent shell is down).
- **USER_MANUAL.md updated** ("Saving & working folder" section) to reflect the post-v43 save workflow: capability badge is now **3-state** (`✓ Direct write` / `⚠ Set folder` / `⚠ Download-only`, ISS-89); Save with no folder opens the standardized Working Folder popup and awaits it — dismiss leaves data unsaved with a toast, no silent download; status bar distinguishes `Saved <time>` (disk) vs `Downloaded <time>` (amber fallback); fresh open shows `Loaded` (ISS-86); Save buttons stay clickable whenever there is an unsaved inline edit (ISS-90 #1) and a slow save captures the latest edit typed mid-flight (ISS-90 #2). The "Browser limits" table and status-bar wording were revised accordingly.
- Workplan status unchanged for v52/v53/v54 (still `pending node --check` + browser smoke test + commit — that verify/commit step is user-side because the sandbox shell is unavailable).
- Related: §5.1 / §6, ISS-86 / ISS-89 / ISS-90.

### [2026-09-11] Issue — ae-log filter / sort toolbar, view-only (ISS-62)
- User approved ISS-62 (first item of Phase D). Added a toolbar above `#aeLog` (inside `.ae-log-wrap`) with Type / Action by / Status / Date-from / Date-to filters and a Date/Type/By/Status sort (asc/desc) + Clear. Hard constraint (from the spec "View-only, no data change"): the feature must not mutate `a.detailLog`, because ISS-61 made "DOM order = data order" (the save path reads DOM order into the data).
- How the constraint is met: filtering hides rows with `display:none` (rows stay in the DOM, so a flush still reads every row — data/content unchanged); sorting reorders the DOM for display only, and `collectLogRows()` restores each row's original `data-i` order whenever a sort is active, so `flushInlineEdits` / `syncPreview` never persist the sorted view; the ↑/↓ manual reorder (ISS-61) is disabled while a sort is active so the two orderings can't conflict.
- `aeLogView` is a module-level pure view state (no persistence); selects repopulate per action's project and drop a saved filter whose option no longer exists in that project (cross-project safety). Clear rebuilds `#aeLogBody` from `a.detailLog` (no flush, no data change).
- Files: `js/render.js` (toolbar HTML + `buildAeLogTools` / `applyAeLogFilterSort` / `rebuildLogBody` / `collectLogRows` + guard in `flushInlineEdits` / `syncPreview`), `css/styles.css` (`.ae-log-tools`). Status: **Implemented (2026-09-11, v55)** — pending `node --check js/render.js` + browser smoke test + commit.
- Related: §6.5 / ae-log, ISS-59 / ISS-61.

### [2026-09-11] Implementation — Export minutes (ISS-63)
- User approved the 4 ISS-63 decisions: (1) button in `#edTop`; (2) Word only; (3) export the **current ISS-62 filter/sort view** (filtered rows + view display order); (4) ISS-63 first, then ISS-64 (`@ref` resolution) stacked on the same export path.
- What it does: an **Export minutes** button (appended to `#edTop` idempotently in `bindActionEditor`) exports the currently-open action's dated detail log as a Word `.doc` — title (h2) + the same 4-row metadata block used by `exportWord` (Project / Discipline / Status / Due / Assigned to / Created by / Created on / Dependencies) + an **extended 6-column log table**: `Date / Type / Action by / Due / Status / Detail` (the existing `detailLogRows` crams these into the Detail cell as sub-labels; ISS-63 spreads them into real columns so the log reads like minutes).
- Reuse: built entirely on the existing export pipeline — `msoDownload(filename, html, mime)` (same Word doctype + Calibri border tables as `exportWord`), and the detail-log field resolvers (`state.actionTypes` → type label, `memberNameById` → action-by name, `statusLabel` → status, `normalizeLinkSrc` + `attachments[]` hyperlink rendering). No new download/copy plumbing.
- View fidelity: `exportOrderedDetailLog(a)` (render.js) reads the **live DOM** rows — skipping `display:none` (filtered-out) rows and reading them in **DOM display order** (which `applyAeLogFilterSort` already reorders to the sorted order) — and pulls the live date input / textarea / Meta-dataset values, so unsaved inline edits are included AND the export reflects exactly what the user is looking at. It never mutates `a.detailLog` (view-only), so it can't affect Save.
- `@ref` deferred to ISS-64: `r.text` is exported as-is (raw). The `expandRefs(text)` hook will plug into `minutesLogRows` once ISS-64 lands.
- Files: `js/render.js` (`exportOrderedDetailLog`, button create + click wiring in `bindActionEditor`), `js/report.js` (`exportMinutes`, `minutesLogRows`, `minutesImgsHtml`). Status: **Implemented (2026-09-11, v56)** — pending `node --check js/render.js js/report.js` + browser smoke test (Word minutes downloads; exports filtered/sorted rows in view order; button appears in #edTop only on the Actions perspective) + commit.
- Related: §6.5 / report, ISS-62 (view state reused), ISS-64 (next, `@ref`).

### [2026-09-18] Bug fix — Word export drops detail-log line breaks (ISS-91)
- User reported (do-not-update investigation) that when exporting actions, the line breaks (Enter) typed in each action's detail were flattened to a single space in the Word file, while the webpage showed them correctly. Root cause confirmed: the export built the detail `<td>` via `esc(r.text)` only — `esc()` escapes HTML special chars but performs NO newline→`<br/>` conversion, and the `<td>` carried no `white-space:pre`; Word follows HTML whitespace rules and collapses every Enter into a space. On-screen rows are `<textarea>` (native newline preservation) and `a.detailLog[i].text` still stores the newline intact, so it was purely an export-rendering gap; Excel export was never affected (flat summary only).
- Fix (user approved: record issue + proceed): in `js/report.js`, both `detailLogRows` (Export actions Word table) and `minutesLogRows` (Export minutes, ISS-63) now emit `esc(r.text||'').replace(/\r\n|\r|\n/g,'<br/>')` — the `<br/>` is inserted AFTER `esc()` so it is not re-escaped, while `& < >` remain safe. On-screen rendering and saved data are untouched.
- Files: `js/report.js`. Status: **Implemented (2026-09-18, v57)** — pending `node --check js/report.js` + browser smoke test (multi-line detail → Word shows each line on its own line in both Export actions and Export minutes; data unchanged) + commit.
- Related: §6.5 / report, ISS-63 (same export path), ISS-64 (`@ref`).

### [2026-09-18] Feature — @ref resolution in Word export (ISS-64)
- User approved the solution (resolved refs highlighted light blue; `@img:` precise case-sensitive name match). Implements lightweight `@ref` inside detail-log text: `@<id>` → action, `@img:<name>` → picture on the same row. Resolution runs **only at export** — `expandRefs(text, row)` is called from `detailLogRows` (Export actions Word) and `minutesLogRows` (Export minutes, ISS-63) with an opt-in `resolve` flag (default off) so on-screen rendering stays raw; the stored `a.detailLog[i].text` is never changed.
- Behaviour: `@<id>` found → `→ #id <title>` (light-blue highlight); found but `a.deleted` → `→ #id <title> [deleted]` (muted); not found → `→ #id [action not found]` (muted, raw token preserved). `@img:<name>` found in the row's `attachments[]` (type image) → `→ IMG <name>` (light-blue, links to the picture src); not found → `→ <name> [image not found]` (muted, mirrors `.broken`). A token boundary guard requires the `@` to follow a space, line start, or `(` so emails like `a@123.com` are never mistaken for a ref. ISS-91's newline→`<br/>` is preserved (applied to plain segments only, never inside resolved spans).
- Files: `js/report.js` (`findActionById`, `resolveActionRef`, `resolveImgRef`, `expandRefs`; `detailLogRows(a, resolve)` / `minutesLogRows(rows, resolve)` now take the flag; `exportWord` / `exportMinutes` pass `true`). Status: **Implemented (2026-09-18, v58)** — pending `node --check js/report.js` + browser smoke test (both Word exports resolve refs; unresolvable muted; data unchanged; Excel unaffected) + commit.
- Related: §6.5 / report, ISS-63 (minutes export path), ISS-91 (newline handling retained).

### [2026-09-18] Feature — show action number on Word export heading (ISS-92)
- User approved Option A: prefix the Word section heading with `#<id> — `. Both `exportWord` (Export actions) and `exportMinutes` (Export minutes, ISS-63) headed each action section by its **title only**, so the subject action's own id never appeared on the page — only the Excel export had an `ID` column (L120–121). Fix: `<h2>` becomes `#${esc(a.id)} — ${esc(a.title)}` in `exportWord` (L114) and `#${esc(a.id)} — ${esc(a.title||'Minutes')}` in `exportMinutes` (L161), matching the ISS-64 `@ref` hash style. No change to the Excel export or stored data.
- Files: `js/report.js`. Status: **Implemented (2026-09-18, v59)** — pending `node --check js/report.js` + browser smoke test (both Word exports show `#id — title`; Excel unaffected; data unchanged) + commit.
- Related: §6.5 / report, ISS-64 (`@ref` hash style reused), ISS-91 (newline handling retained).
...
| `arch/v37_apply.md` | Manual find→replace apply guide for the v37 layout-polish batch | 🗄️ Archived 2026-09-10 (long applied) |
| `arch/v38_apply.md` | Manual find→replace apply guide for the v38 defect batch | 🗄️ Archived 2026-09-10 (long applied) |
| `arch/apply_v38.js` | Idempotent Node patch script for the v38 defect batch | 🗄️ Archived 2026-09-10 (long applied) |
| `arch/OVERVIEW.md` | Stray session note (wrong username path) — intermediate, not needed | 🗄️ Archived 2026-09-10 |
| `USER_MANUAL.md` | Editable source for the run/save instructions (markdown) | ✅ Current (editable source) |
| `USER_MANUAL.html` | **Static, offline-friendly HTML user manual** generated from `USER_MANUAL.md` — self-contained (inline CSS), Apple-style design tokens, light+dark via `prefers-color-scheme`, sticky jump-nav, styled tables/code/badges, mobile-first responsive. Mirrors `USER_MANUAL.md` (run cases, saving & working-folder workflow incl. ISS-86/89/90). Keep the two in sync when the save workflow changes. | ✅ Created 2026-09-11 |
...
| ISS-62 | ae-log P3 — filter/sort toolbar above `#aeLog` (by `typeIds` / `actionBy` / `status` / `date`; asc/desc). View-only, no data change. | §6.5 / ae-log | **Implemented (2026-09-11, v55)** — pending `node --check js/render.js` + browser smoke test (filters hide non-matching rows; sort reorders view only; Save never persists sort order; ↑/↓ disabled while sorted) + commit |
| ISS-63 | ae-log P4 — "Export minutes" button reusing `reportHtml` / `detailLogRows` to emit a minutes view (title + metadata + extended log table: Date / Type / Action by / Due / Status / Detail) via existing `msoDownload` (Word / Excel). | §6.5 / report | **Implemented (2026-09-11, v56)** — pending `node --check js/render.js js/report.js` + browser smoke test (Word minutes; exports current ISS-62 filter/sort view; button appended to #edTop in bindActionEditor) + commit |
| ISS-64 | ae-log P5 — lightweight `@ref` in `detailLog[i].text` (`@<id>`→action, `@img:<name>`→picture); resolved into readable text **only at export** (ISS-63) via `expandRefs(text)`; never silently dropped (raw ref retained); missing ref → muted `→ #id [action not found]`; deleted action → `→ #id <title> [deleted]`; broken image refs mirror existing `.broken` handling. | §6.5 / ae-log | **Implemented (2026-09-18, v58)** — pending `node --check js/report.js` + browser smoke test (both Word exports resolve @ref; unresolvable muted; data unchanged; Excel unaffected) + commit |
| ISS-92 | ae-log — show the action's own number on the Word export heading. Export actions (`exportWord`) and Export minutes (`exportMinutes`) headed each section by title only, so the subject action's id was absent from the page (only Excel had an `ID` column). Resolution: prefix the section `<h2>` with `#<id> — ` (matches the ISS-64 `@ref` hash style). | §6.5 / report | **Implemented (2026-09-18, v59)** — pending `node --check js/report.js` + browser smoke test (both Word exports show `#id — title`; Excel unaffected; data unchanged) + commit |
...
| ISS-78 | ae-log — **Attachment popover "Card C" hybrid layout** (2026-09-08, supersedes the loose inline layout from ISS-77): user design review of `#aeAttachPop` (2026-09-08, Chinese) found (1) two tip-text bugs — `C:\folder` was written inside a JS template literal so `\f` became a form-feed, and the tip falsely promised a `file://` link "opens"; (2) inconsistent alignment between the Pictures and File-links groups; (3) the core `file://` limitation. User then evaluated an "all buttons at the bottom" variant and approved the agent's **Card C** recommendation (content grouped inline, **only the Done button in the footer** — avoids the GitLab ≤3-action footer anti-pattern; references GitLab/Primer/macOS modal conventions), then: "use your recommendation C." Implemented: `openAttachPop` body rebuilt as two cards (`.ae-attach-card`) — **Pictures** (`+ Add pictures` in the header) and **File links** (`Browse files` in the header; paste-URL + `Add link` row; an honest tip). Cards stay visible when empty (so the user can always add) and auto-collapse their chip row when empty. Tip: the `C:\folder` example is now correctly escaped (`C:\\folder\\file.pdf`) and the tip honestly states a `file://` link opens only when the app runs as a local file, recommending a relative path (e.g. `files/report.pdf`) for server use. `renderChips` unchanged (still toggles `has-items`/`has-any` on the retained `.ae-attach-group` class). Files: `js/render.js`, `css/styles.css`. | §6.5 / ae-log | **Implemented (2026-09-08)** — pending `node --check js/render.js` + browser smoke test (two cards; header actions right-aligned; only Done in footer; empty cards visible; tip text correct + honest `file://` guidance) + commit |
| ISS-79 | tree — **Show action number (`#id`) in the left tree node** (2026-09-08): user asked the action's stable `id` to appear in the Actions tree so a node can be correlated with the open action (subhead `#id`), dependency references, and exports without opening it. Earlier the design was proposed and confirmed (Action ID `#NN`, placed **left of the title**). Implemented in `renderActionNode()` (`js/render.js`): a `<span class="tnum">#${a.id}</span>` is emitted immediately after the chevron/dot and before the title. CSS `.tnum` added in `css/styles.css` (muted text on `--chip` at rest; solid `--accent` + white text when the row is selected, matching the `.sel` light-tint treatment). Applies to every action row (top-level and nested sub-actions); parent rows keep their `∑ N` rollup badge; works in filtered views; no data-model or `io.js` migration needed. | §6.5 / tree | **Implemented (2026-09-08)** — pending `node --check js/render.js` + browser smoke test (#id shows left of each title; parents still show `∑ N`; selected row highlights chip in accent; tracks new/reordered actions) + commit |

| ISS-80 | Save/Working-folder workflow + standardized user-message popups (2026-09-02): (1) folder known → Save writes directly into it, no re-prompt; folder unknown → ask to choose; (2) stay on same page, no auto-save; (3) any drive, not locked to Z:; (4) status bar shows working-folder set/unset; (5) top-bar Save buttons removed — Save lives in right-panel Quick Actions + Ctrl/Cmd+S; (6) status bar shows browser-limit (capability) badge; (7) folder button clearly means "set folder, not save"; (8) no folder → open picker then save immediately; (9) standardized `#appMsg` popup (`appAlert` OK-only / `appConfirm` OK+Cancel) replaces native `confirm()`/`alert()`; (10) navigation guard blocks panel switch when dirty and informs "Save before switching to another panel" (OK alone sufficient; Cancel meaningless). New helpers: `writeIntoFolder(folder,name,text)`, `saveTimeNow()`, `openWfPop`, `appAlert`, `appConfirm`; `state.lastSavedAt`; `updateStatusbar` rewrote to show folder + save-time + amber unsaved dot + capability badge; `#wfPop`/`#appMsg` modals; `USER_MANUAL.md` "Saving & working folder" section. Files: `js/core.js`, `js/io.js`, `js/bootstrap.js`, `css/styles.css`, `index.html`, `USER_MANUAL.md`. | §5.1 / §5.5 | **Implemented (2026-09-02, v43)** — pending `node --check` (core.js/io.js/bootstrap.js) + browser smoke test + commit |
| ISS-81 | **Remove the editor-top inline Save button** (`#aeSave` in `#edTop`): user reported a "Save" button still visible at the top of the main panel (the inline action editor header). It was the only commit path for inline edits (it wrote the DOM edits into the action object AND called `writeDataFile`), so simply deleting it would lose unsaved inline edits. Fix: removed the button; added `flushInlineEdits()` that commits pending inline DOM edits into the action object (no file write) and is invoked automatically before a global Save (`writeDataFile`) and before re-rendering (`renderActionsMain`), so no edit is lost. `bindActionEditor` records `_openActionId` so flushInlineEdits targets the correct action even after `state.selection.actions` has changed. The right-panel **Save Actions** / **Save Settings** + Ctrl/Cmd+S remain the single save entry point. | §6.5 / §5.1 | **Implemented (2026-09-09, v44)** — pending `node --check js/render.js io.js` + browser smoke test + commit |
| ISS-82 | **Working Folder popup shows the default deploy path** (2026-09-09): user asked to surface the launcher's deploy target so the status-bar folder name can be reconciled with the real on-disk location. `openWfPop` (core.js) now renders a `.wf-deploy-hint` paragraph: "Default deploy folder: `Z:\7. Engineering\01-Eng Mgmt\01 Action Log` — the launcher serves the app from here. Save writes to the working folder you choose above, which can be a different drive." The path is a read-only reference (browser security still prevents the app from reading the real OS path of a *chosen* folder); it does not participate in the save logic. CSS `.wf-deploy-hint` + `.wf-deploy-hint code` added in styles.css. | §5.1 / §5.5 | **Implemented (2026-09-09, v45)** — pending `node --check js/core.js` + browser smoke test (popup shows deploy path; Choose folder still works) + commit |
| ISS-83 | **Working Folder popup — click-to-copy the deploy path** (2026-09-09): extends ISS-82 per user request ("yes, make it click-to-copy"). The deploy path in `openWfPop` is now interactive: clicking the path text (`#wfDeployPath`) or the **Copy** button (`#wfCopyPath`) copies the full path to the clipboard via a new `copyText()` helper; on success a `✓ Copied` label (`#wfCopyOk`) flashes for ~1.6s and a toast confirms. `copyText()` prefers `navigator.clipboard.writeText` (secure contexts incl. `http://localhost`) and falls back to a hidden `<textarea>` + `execCommand('copy')` for Firefox/`file://` (no clipboard read error). Path stored once in the `DEPLOY_PATH` constant. CSS `.wf-path` (clickable + accent hover), `.wf-copy`, `.wf-copy-ok` added in styles.css. | §5.1 / §5.5 | **Implemented (2026-09-09, v46)** — pending `node --check js/core.js` + browser smoke test (click path or Copy → clipboard has full path; ✓ Copied flashes; Choose folder still works) + commit |
| ISS-84 | **File-link label drops `file:///` + `%20`** (2026-09-09): bug — pasting a `file:///` folder URL (e.g. `file:///Z:/7.%20Engineering/01-Eng%20Mgmt/03%20Master%20List/01-PPP`) into the attachment popover's "Add link" box rendered a link whose chip **tooltip** and stored `name` still exposed the raw `file:///` prefix and `%20` escapes. Fix: new `prettyLinkName(src)` (render.js) strips the `file://` scheme, `decodeURIComponent`s `%20`→space, restores backslashes → `Z:\7. Engineering\01-Eng Mgmt\03 Master List\01-PPP`; web/blob/data keep last-segment. "Add link" now stores `name: prettyLinkName(v)`; `renderChips` tooltip uses `prettyLinkName(im.src)` — so text/tooltip never show `file:///` or `%20`. The **href** still uses `normalizeLinkSrc` (valid `file://` URL) so the folder opens on click. `normalizeLinkSrc` also `%20`-encodes literal spaces in the Windows-path branch for reliable opening. | §6.5 / ae-log | **Implemented (2026-09-09, v47)** — pending `node --check js/render.js` + browser smoke test (paste file:// folder URL → chip shows clean path + opens on click; no `file://`/`%20` in text/tooltip) + commit |

| ISS-85 | Save-lifecycle feedback + status-bar "unsaved" fix (2026-09-08): user reported (a) no "saving in progress" indicator when Saving over a VPN-mapped network drive, allowing repeated clicks during the slow write, and (b) the status bar did not show "● unsaved" on changes. Root causes: (a) `writeDataFile`/`saveSetupFile` are async with no visual state and save buttons only disable on dirty (re-entry possible); (b) `updateStatusbar()` was only called on save-complete/folder-set/auto-load while the dirty setters (`markDirty`/`markDataDirty`/`markSetupDirty`) only called `updateSaveButtons()`; `syncTopbarUnsaved` omitted `edDirty`. Fix: a `saving` flag guards re-entry (`if(saving) return`) and drives a blue pulsing dot + "Saving…" spinner (status bar) + a spinner on all four save buttons (`pointer-events:none` keeps them full-opacity, blocks re-click); `updateStatusbar()` now fires on every dirty change so "● unsaved" appears immediately; three-state CSS — Saving (accent + spinner) > `⚠ Save failed` (red, auto-clears 4s) > `● unsaved` (amber) — with `prefers-reduced-motion` support; `syncTopbarUnsaved` now includes `edDirty`; dead `#aeSave` code removed from `updateSaveButtons`. New `setSaving(on)` / `showSaveError(msg)`. Files: `js/core.js`, `js/io.js`, `js/render.js`, `css/styles.css`. | §5.1 / §6.5 | **Implemented (2026-09-08, v48)** — pending `node --check` (core.js/io.js/render.js) + browser smoke test (edit → status bar "● unsaved" at once; Save over slow VPN shows blue "Saving…" + button spinner, no duplicate save; success → "Saved <time>"; failure → red "⚠ Save failed"; top-bar "● unsaved" reflects inline edits) + commit |
| ISS-86 | Status bar mislabels fresh-open as "Not saved yet" (2026-09-09): on first open the save-time chip read "Not saved yet" even though a file was just loaded (in-memory data == on-disk baseline), wrongly implying pending unsaved edits. Root cause: `savedTxt` in `updateStatusbar()` fell back to "Not saved yet" whenever `state.lastSavedAt` was empty, but `lastSavedAt` is only set after an actual Save — so every fresh open (source external/imported/embedded) showed "Not saved yet". Fix: `savedTxt` returns "Loaded" when a dataset was loaded (`src !== 'none'`) and `lastSavedAt` is empty; "Not saved yet" is reserved for the genuine empty state (`src === 'none'`). Chip gets a green `.sb-loaded` style (same palette as `sb-ext`/`sb-cap-ok`) + tooltip "Loaded from file — matches disk, no unsaved changes". Files: `js/io.js`, `css/styles.css`. | §5.1 / §6 | **Implemented (2026-09-09, v49)** — pending `node --check js/io.js` + browser smoke test (fresh open of action.json → green "Loaded", no "● unsaved"; truly empty/no-file start → still "Not saved yet"; after Save → "Saved <time>") + commit |
| ISS-87 | Status bar reorganised into per-message panels (2026-09-09): user review (do-not-update pass) flagged the bar as a flat run of unlabeled fragments ("from file"/"Working Folder/File/Status" noise) that read as a jumble. Follow-up to ISS-85/86: each status message is now its own bordered **panel** on the (dark) bar — `[file]` (coloured source dot + filename) · `[folder]` (SVG folder icon replacing the emoji + working-folder name) · `[save-state]` (one unambiguous phrase: `Saving…` / `Save failed` / `● N unsaved` / `Loaded` / `Saved <time>`, with `from file` as a muted suffix) · `[counts]`. The browser-capability badge ("direct disk write"/"download-only") moves into the dot's tooltip; the migration tag (`⚠ legacy → v1`) becomes its own orange panel. Panels are translucent chips (rgba white 0.06 / border 0.14 / radius 7px) suited to the dark bar; folder panel flexes with ellipsis, counts panel pushed right (`margin-left:auto`). `max-width:900px` media query hides folder + counts first so save-state never clips. `updateStatusbar()` (io.js) rewritten to emit the panel HTML; `.sb-panel`/`.sb-id`/`.sb-loc`/`.sb-folder`/`.sb-folder-ico`/`.sb-state`/`.sb-state-txt`/`.sb-src-suffix`/`.sb-meta`/`.sb-mig` styles added, `#statusbar` gap tightened to 8px. **Refinement (2026-09-09, same v50):** user reported the folder panel stretched too long and asked all panels to auto-fit, and told me NOT to delete the direct-disk-write message. Fix: every panel is now `flex:none` (auto-fits content, no stretch); the folder panel is capped (`max-width:240px` with the name ellipsis-capped at `max-width:200px`) so a long folder name no longer blows it up; the capability badge ("✓ Direct write" / "⚠ Download-only") is **restored as its own `.sb-cap` panel** (previously folded into the dot tooltip in the first v50 cut) — placed right before the counts panel, coloured green/amber by `.sb-cap-ok`/`.sb-cap-no`. `#statusbar` switched to `justify-content:flex-start` (panels group left, counts pushed right via `margin-left:auto`); the narrow-hide threshold raised to 680px since the folder now ellipsizes instead of stretching. Files: `js/io.js`, `css/styles.css`. | §5.1 / §6 | **Implemented (2026-09-09, v50)** — pending `node --check js/io.js` + browser smoke test (4–5 distinct panels, all auto-fit; folder name ellipsizes instead of stretching; capability panel shows "✓ Direct write"/"⚠ Download-only"; fresh open → green "Loaded · from file"; edit → amber "● N unsaved"; Save → blue "Saving…" + button spinner; success → "Saved <time>"; narrow ≤680px → folder+counts drop, save-state stays) + commit |

| ISS-88 | First Save fails with no fallback / data loss (2026-09-09): user reported that after a fresh load (status bar shows `✓ Direct write` because `window.showDirectoryPicker` exists) the **first Save Actions** shows `Save failed: <details>` and writes nothing. Root causes: (1) the `✓ Direct write` capability panel only reflects **browser** support, NOT that a writable working folder exists — `dataDirHandle` is `null` on a fresh load, so the first Save prompts `setDataFolder`; (2) `loadDataDir()` (bootstrap, line 59) restores a **previously chosen** folder handle (e.g. a VPN/network-mapped drive) into `dataDirHandle` *before* the first Save, so the direct-write branch runs even with no explicit folder pick; (3) when `writeIntoFolder` then throws (unreachable/read-only network folder, the exact VPN scenario from ISS-85), all four write-failure branches only called `showSaveError('Save failed: …')` and **gave up — the data was never written anywhere**. Fix: every `writeIntoFolder` failure in `writeDataFile`/`saveSetupFile` (both the pre-set-folder branch and the just-chosen-folder branch) now **falls back to `downloadJson`** so the user always gets a file, with a clear toast naming the cause ("Direct write failed (…) — downloaded action.json/setup.json instead (set a Working Folder on this PC for disk writes)") and clears the error/dirty state. No more silent data loss on a failed direct write. Files: `js/io.js`. | §5.1 / §6 | **Implemented (2026-09-09, v51)** — pending `node --check js/io.js` + browser smoke test (pick a VPN/network folder as Working Folder → Save Actions → instead of "Save failed" with no file, a download of action.json fires with a clear cause toast; normal local folder → still writes directly; cancel folder picker → download fallback; status bar clears to "Saved <time>") + commit |

| ISS-89 | Save workflow unified + capability 3-state + save-result wording (2026-09-10): user reported (a) status bar showed `✓ Direct write` on first load even with no folder set, (b) in Edge the first Save only downloaded, (c) picking a working folder triggered a non-standard system permission window. Review (do-not-update pass) found P1 (capability panel shows false green before a folder exists), P2 (Save hijacks the raw folder picker instead of the standardized popup), P3 (two visual experiences for the same action), P4 (native permission prompt feels like a stray window), P5 (status bar can't tell disk-write from download). Fix (A–D): (A) capability panel 3-state — folder set → green `✓ Direct write`; API supported but no folder → amber `⚠ Set folder`; no API → amber `⚠ Download-only`; (B) Save with no folder opens `openWfPop` and awaits it — dismiss leaves data unsaved with a toast, no silent download; (C) `openWfPop` now explains the upcoming native browser permission prompt (`.wf-perm-hint`) so it no longer reads as a stray window; (D) status bar distinguishes `Saved <time>` (wrote to disk) vs `Downloaded <time>` (amber, fell back to download) via new `state.lastSavedVia`. `openWfPop` returns a Promise resolved by a MutationObserver on popup close (robust to every close path, so Save never hangs); `setDataFolder` returns a boolean; all three folder entry points (rpFolder/tbFolder/dtFolder) use `openWfPop`. Files: `js/core.js`, `js/io.js`, `js/settings.js`, `css/styles.css`. | §5.1 / §6 | **Implemented (2026-09-10, v52)** — pending `node --check` (core.js/io.js/settings.js) + browser smoke test (fresh Edge open → amber `⚠ Set folder`; Save → standardized popup not raw picker; dismiss → no download + toast "Working folder not set"; choose folder → direct write; native permission prompt explained in popup; bar shows `Saved` vs `Downloaded`) + commit |

| ISS-90 | Save buttons disabled while status bar shows "● unsaved" (save-lifecycle hardening, 2026-09-10): during a slow in-flight save (VPN/network-folder `writeIntoFolder`, or while the `openWfPop` working-folder popup is open), `flushInlineEdits()` commits the DOM into the action object **before** the `await`, but `writeDataFile`/`saveSetupFile` clear `state.dataDirty` in their `finally`-block; a new inline edit typed during the `await` leaves `edDirty=true, dataDirty=false`. `updateSaveButtons()` gated Save Actions on `state.dataDirty` alone, so the button ended up **disabled** while the status bar showed `● unsaved` — an inconsistent state with a silent-loss path (switching panel flushes the DOM edit into `a` without re-dirtying → edit never saved, no `beforeunload` warning on close). Note: `saving` and the dirty flags are orthogonal (saving never reads/writes them and always resets via `try/finally`), so a genuine permanent deadlock does NOT occur. Fix (#1, v53): Save Actions (`tbSaveActions`/`rpSaveActions`) now enable on `state.dataDirty || edDirty`, closing the inconsistent state + silent-loss path. (#2 — re-serialize after the `await` so a slow save captures the latest edit, not the pre-edit snapshot — **implemented 2026-09-10, v54**.) Files: `js/core.js` (`updateSaveButtons` #1), `js/io.js` (`writeDataFile` #2). | §5.1 / §6 | **Implemented (2026-09-10, v53 + v54)** — pending `node --check` (core.js/io.js) + browser smoke test (slow Save + edit mid-flight → Save Actions stays clickable AND the saved file contains the latest edit) + commit |

| ISS-91 | ae-log — Word export flattens each action's detail-log line breaks into a single space. Root cause: the generated `<td>` text used only `esc(r.text)`, which escapes HTML special chars but performs no newline→`<br/>` conversion, and the `<td>` carried no `white-space:pre`; Word follows HTML whitespace rules and collapses every Enter into a space. On-screen rows are `<textarea>` (native newline preservation) and `a.detailLog[i].text` still stores the newline intact, so only the export was wrong; Excel export writes only the flat summary and was never affected. Fix: in both `detailLogRows` (exportWord) and `minutesLogRows` (exportMinutes) emit `esc(r.text||'').replace(/\r\n|\r|\n/g,'<br/>')` (added AFTER `esc()` so the `<br/>` is not re-escaped; `& < >` stay safe). Files: `js/report.js`. | §6.5 / report | **Implemented (2026-09-18, v57)** — pending `node --check js/report.js` + browser smoke test (multi-line detail → Word shows each line on its own line, in both Export actions and Export minutes; data unchanged) + commit |
