...
- **Share link:** https://889d525c87954023a62ef99476da83bd.app.workbuddy.link/
  - This is the **current deployed link** — it serves the latest build (service worker caches `ed-app-v36` + `ed-content-v1`: **load-time fix batch.** Three separate causes of the slow first paint were found and fixed. (1) **The waterfall**: `loadContent()` awaited `manifest.json` plus all **eight** packs one after another before rendering anything. Measured against the live host, a single request costs 2–10 s TTFB (example.com on the same machine: 0.27 s), so that was ~9 serial round trips. `loadContent()` now fetches only the **active** pack before the first paint (the manifest already lists every id, so the choice is made up front) and hands the rest to `loadRemainingPacks()`, which runs them **in parallel** in the background; when they land it rebuilds `CROSS_DICT`, invalidates the word/learned caches and re-paints only the panels that cannot interrupt a session (`renderPlan`, `renderMe`, reading if unanswered, review/practice if no queue is running). `packsReady` is exposed so the pack `<select>` waits for a not-yet-loaded pack instead of silently doing nothing. Measured in jsdom: first paint 446 ms → 310 ms locally, and on a slow host ~9 serial round trips → 2. (2) **Sanitising trusted packs**: `normalizePack()` ran `sanitizeRichText()` over all ~94 readings of our own `./content/` packs on every boot — 83 ms measured, several hundred ms on a phone, for zero benefit. It now takes an `untrusted` flag and only imported/user-supplied packs are sanitised. (3) **The service worker re-downloaded everything on every deploy**: one shared `CACHE` name meant each version bump wiped ~2.4 MB and the phone re-downloaded all of it. `sw.js` now keeps `APP_CACHE` (`ed-app-vN`, bumped per deploy, ~200 KB) separate from `CONTENT_CACHE` (`ed-content-v1`, 2.2 MB, bumped only when a pack actually changes), topping the content cache up with `cache.match()` before `add()`. Old caches are still deleted on activate. New check script `build/_check_async_packs.js` (jsdom functional smoke: 14 assertions) plus `build/_perf_probe.js` (boot phase profiler) and `build/_perf_server.js`. Regression: all four earlier check scripts still pass. Underlying `ed-v35-1790440000`: **code-review fix batch (5 Major + 9 Minor).** M-2 **guarded persistence**: `saveState()` now try/catches every write (it had 29 call sites and none were guarded — a `QuotaExceededError` killed the rest of the caller so a learned word was silently lost on the next cold start), and the imported content pack moved to its own key `englishDaily_pack_v1` so a ~700 KB pack can no longer take the progress record down with it (`loadState()` reads the new key, falling back to the legacy copy inside the old blob). M-1 **reading highlight**: `highlightPriorWords()` rebuilt and re-tested ~4 900 RegExp objects per text node (measured 18.6 ms/node → seconds on a phone); it now tokenises with `/[A-Za-z]+(?:-[A-Za-z]+)*/g` and does O(1) hash lookups (measured 186 ms → 0 ms for 10 nodes), and the `</?b` string sniff that aborted every later key — so only ONE prior word per node was ever highlighted — is replaced by a structural `NodeFilter` that skips text already inside `<b>`. M-3 **imported packs are sanitised**: `sanitizeRichText()` keeps a tag whitelist (`B/STRONG/I/EM/U/BR/P`, so the blue-word mechanism still works) and drops every attribute, killing `<script>` / `<img onerror>` / `<svg onload>`; `esc()` added to the pack card, the Me bookmarks list, the quiz card and the reading title/level/question. M-4 **`clearUndo()`** runs on every tab switch and the 5 s undo timer only repaints when a review-ish tab is still active (it used to overwrite the Practice listen/quiz card). M-5 **`saveCardSettings()` no longer calls `recordActivity("review")`** — tweaking a card setting must not extend the streak. Minor batch: reminder now compares total minutes (21:00 vs 20:30 never fired) and fires once per day; "全部答对" is only toasted when it is true; `setupA11y()` no longer clobbers every tab's `aria-controls`; orphan `data-act="reportEmpty"` removed; unreachable `skip` branch in `getReviewType()` removed; `learnedAny()` uses the cached key set and `findWordGlobal()` uses a built index; `rateCard()` guards a non-numeric `interval` (NaN made a card vanish forever); focus trap covers both modals; `sw.js` caches assets individually instead of the atomic `addAll()`. New i18n: `toast_save_fail`, `toast_save_pack_fail`. New check script `build/_check_review_fixes.js`. Underlying `ed-v34`: **fixed a duplicated 🔊 on the Me-page 「朗读速度 / Reading Speed」 card.** The button markup carried a hardcoded `🔊 ` prefix while the `speech_test` i18n value also begins with `🔊`, so every `applyI18n()` pass rendered `🔊 🔊 试听效果`. The hardcoded prefix was removed; the emoji now comes only from the i18n string for both zh and en. A repo-wide scan for the same class of bug (hardcoded emoji immediately before a `[data-i18n]` span whose dictionary value also starts with an emoji) found `speech_test` as the only occurrence. Underlying `ed-v33`: **flash card is now the default review method.** A single constant `DEFAULT_REVIEW_TYPE = "flash_only"` drives it: `migrateState` backfills every never-configured card to `flash_only` (was `auto`), `getInitialReviewType()` returns it for new cards, `stepsFor()` falls back to it when a card has no `reviewType`, the ⚙️ sheet pre-selects 🔁 instead of "auto", and un-skipping restores it. **"auto" is deliberately kept as an opt-in** — if a user picks it in ⚙️, the old adaptive engine (pos / reps / interval / ease / proficiency) still runs, and `getReviewType()` continues to preserve any non-auto override so rating a card never downgrades it. The legacy initial-type heuristics were kept verbatim as `_getInitialReviewTypeLegacy()` in case we ever want them back. Underlying `ed-v32`: **one-tap ⏭️ skip button on every card surface.** A shared `skipToggleHTML(word)` renders a `⏭️` / `↩️` toggle (highlighted with `.iconbtn.on` when skipped) on the Daily word card, the Reading Key-Words chips, the flash card's rating row, the typed-result card, and both the productive and context cards — 6 render sites, so a word never has to be skipped through the ⚙️ sheet. `case "toggleSkip"` flips `state.flashcards[k].skipped`, sets `reviewType` to `skip`/`auto`, and when skipping also **removes the word from the running `currentQueue`** so the card leaves the session immediately instead of reappearing. New i18n: `skip_word`, `resume_word`, `toast_skip`. Underlying `ed-v31`: **"⏭️ 跳过 / skip" is now a live setting, not a dead flag.** Previously `saveCardSettings()` wrote `state.flashcards[key].skipped = true` but nothing ever read it, so a skipped word kept coming back on its due date. Now `isSkipped()` / `notSkipped()` gate every drill surface: `dueCards()` and `dueCardsGlobal()` drop skipped cards from the SRS queue, `buildQueue()` drops them from the all-flash and free-practice pools, and `buildQuiz()` drops them from BOTH quiz pools (this pack and all packs) — including the distractor options, which now draw from the same filtered `wordPool`. Skipped words are still counted in `flashActive()` / Me stats so numbers never shrink unexpectedly. Recovery path: a new Me-page card `⏭️ 已跳过的词 (Skipped)`, hidden when empty, listing each skipped word with 🔊 and a `↩️` restore button (`case "unskip"` → `skipped=false`, `reviewType="auto"`). New i18n keys: `me_skipped`, `skipped_note`, `toast_unskip`. Underlying `ed-v30`: **Reading cadence — optional "每天一篇 (daily)" free reading.** A new `.scope-row#readingCadenceRow` sits under the Reading intro with two tabs (`每周一篇` / `每天一篇`), backed by `state.readingCadence` (default `weekly`, so existing behaviour is unchanged). In `daily` mode `refreshReading()` picks the passage with `dailyPickIndex()` — a stateless, per-pack deterministic permutation walked one slot per `dayIndex()`, so a 12-reading pack shows 12 different passages in 12 days before cycling. **Daily reading is explicitly progress-free:** no `readingsDone` write, no `recordActivity`, no streak, no Me stats; the `标记已读` button is replaced by a `reading_free_note` line and `completeReading` is guarded against the daily branch. `currentReading()` was deliberately left untouched so `todaysWords()`, streak and stats keep their weekly rhythm. Comprehension questions stay (they only mutate DOM), and Key Words "学" (`learnFromReading`) still works because it is a user-initiated action. `readingAnswer()` now reads `readingState.idx` instead of recomputing the weekly index, and word popups resolve against `displayedReading()`. The `+ listenCardHTML()` append that used to sit at the bottom of every flash / productive / context card in `renderReview()` is gone — Review tab is now exclusively flash + stepped practice, no auto-attached listening player. Practice → "全部单词 (allflash)" follows the same path so it shows ONLY flash cards too (no surprise listen section). Listen & Learn is still reachable from the Practice tab as `state.practiceMode === "listen"` — that path now owns the whole `practiceArea` with its own dedicated `listenCardHTML()` UI. Underlying P1 nav refactor still from `ed-v27`; the closure-TDZ `state.practiceMode` migration from `ed-v28`. P0 reviewType state machine from `ed-v26`. Full Phase-2 example rewrite §12.1, global-SRS refactor, Spell It + Context modes, status colors, B1–B6 accessibility fixes, Option F. Re-deploys reuse the same link. After a deploy, close and reopen the app once so Safari picks up the new service worker.
